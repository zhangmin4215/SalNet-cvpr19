...
parser.add_argument("-e","--episode",type = int, default= 7500)
...
def phase_correlation(a, b):
    ...
    R = G_a*conj_b
    R /= np.absolute(R)
    score = np.fft.ifft2(R).real
    return score

def power_norm(x, SIGMA):
	out = 2*torch.sigmoid(SIGMA*x) - 1
	return out
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        support_dataloader = tg.get_mini_imagenet_data_loader(task,num_per_class=SUPPORT_NUM_PER_CLASS,split="test",shuffle=False)
        query_dataloader = tg.get_mini_imagenet_data_loader(task,num_per_class=BATCH_NUM_PER_CLASS,split="train",shuffle=True)
        ...
        similarity_measure = phase_correlation(support_foreground_features.cpu().detach().numpy(), query_foreground_features.cpu().detach().numpy())
        
        support_mix_features = mixture_network((support_foreground_features + support_background_features).view((CLASS_NUM**2)*(SUPPORT_NUM_PER_CLASS**2),64,19,19)).view(CLASS_NUM,SUPPORT_NUM_PER_CLASS,-1,64,19**2).view(5,1,5,64,19,19)
        support_mix_features = (support_mix_features.cpu().detach().numpy()*similarity_measure).sum(2).sum(1)
        #support_mix_features = similarity_measure.sum(2).sum(1)
        ...
        query_dataloader = tg.get_mini_imagenet_data_loader(task,num_per_class=BATCH_NUM_PER_CLASS,split="train",shuffle=False)##
        query_img,query_sal,query_labels = query_dataloader.__iter__().next()##
        ...
        if np.mod(episode,2500)==0:
            ...
            for i in range(TEST_EPISODE):
                ...
                support_dataloader = tg.get_mini_imagenet_data_loader(task,num_per_class=SUPPORT_NUM_PER_CLASS,split="test",shuffle=False)
                num_per_class = 2
                query_dataloader = tg.get_mini_imagenet_data_loader(task,num_per_class=num_per_class,split="train",shuffle=True)
                support_img,support_sal,support_labels = support_dataloader.__iter__().next()
